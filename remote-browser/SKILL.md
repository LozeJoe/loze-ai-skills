...
```bash
# Set browser geolocation for location-aware sites (testing/dev)
...
```bash
# Readability mode (default) — keeps the main article body
...
- **Readability vs raw** — `/text` (default) returns the main article body; `?mode=raw` returns full `innerText`.
